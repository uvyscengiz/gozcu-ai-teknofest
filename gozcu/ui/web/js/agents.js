// Ajan mimarisi gorünümü — düğümler, kablolar ve üzerlerinde akan paketler.
//
...
//
// **Akış iki kipte çalışıyor:**
//
// 1. **Demo kipi** — sayfa açıldığında: paketler rastgele seçilmiş kenarlarda
//    yürüyor, hiçbir gerçek devir kaydını okumuyor. `#agentsFlowMode` rozeti
//    "demo akışı" diyor. Ölçülmemiş bir şeyi ölçülmüş gibi göstermek,
//    jürinin önünde cevaplanamayan bir soru bırakır.
//
// 2. **Canlı kip** — `setRunId()` çağrıldığında: `/api/run/{id}/handoffs`
//    ucu yoklanıyor ve gerçek devir kayıtları paket olarak yürütülüyor.
//    Rozet "canlı devirler"e dönüyor. Geçiş `sse.js`'ten geliyor.
//
// Düğüm kimlikleri koddaki ajan adlarıyla BİREBİR aynı
...
export function createAgents({ svgEl, modeEl, legendEl, tipEl, zoomEls }) {
  if (!svgEl) {
    return {
      start() {}, stop() {}, pushHandoffs() {},
      setMeta() {}, setRunId() {}, applyState() {},
    };
  }
  ...
  // Canlı kip durumu — `setRunId` gelince demo durur, gerçek devir verisi
  // `/api/run/{id}/handoffs`'tan çekilir.
  let runId = null;
  let liveMode = false;
  ...
  // ===========================================================================
  // Canlı kip — gerçek devir verisi (`/api/run/{id}/handoffs`)
  // ===========================================================================
  //
  // `setRunId` geldiğinde demo durur ve rozet "canlı devirler"e döner.
  // `applyState` her SSE çerçevesinde çağrılıyor; devir defteri koşu
  // ilerledikçe büyüyor, ekran da onunla büyümeli.
  ...
  function switchToLive() {
    ...
    stopDemo();
    if (modeEl) {
      modeEl.textContent = "canli devirler";
      modeEl.dataset.state = "live";
      modeEl.title = "Kablolardaki hareket GERÇEK devir kayıtlarından geliyor "
        + "(/api/run/{id}/handoffs).";
    }
  }
  ...
  function switchToDemo() {
    ...
    if (running) scheduleDemo();
  }

  /** Devir defterini sunucudan çeker ve yeni satırları paket olarak
   *  yürütür. Önceki çekimde görülen satırlar yeniden gönderilmiyor. */
  let lastHandoffCount = 0;
  ...
  async function refresh() {
    ...
    try {
      ...
      // Yalnız yeni satırları paket olarak gönder — daha önce gösterilenleri
      // tekrar yürütmek ekranı anlamsız hareketle doldururdu.
      const fresh = rows.slice(lastHandoffCount);
      lastHandoffCount = rows.length;
      fresh.forEach((row) => sendPacket(row.source, row.target, "chain"));
    } catch {
      return;                    /* cekilemezse önceki durum ekranda kalır */
    }
  }
  ...
  return {
    ...
    /** Gerçek devir kayıtlarını akışa çevirir.
     *
     *  `/api/run/{id}/handoffs`'tan gelen satırları buraya vermek de
     *  mümkün; `setRunId` + `applyState` üzerinden otomatik çekiliyor
     *  ama dışarıdan da beslenebilir. */
    pushHandoffs(rows) {
      (rows || []).forEach((row) => sendPacket(row.source, row.target, "chain"));
    },

    // =========================================================================
    // PR #3'ten gelen entegrasyon arayüzü — `sse.js` bu üçünü çağırıyor.
    // =========================================================================

    /** Meta bilgisi geldiğinde çağrılır. PR #4'ün kendi meta verisi
     *  (`AGENTS[]`) var; bu metot uyumluluk için tutuldu — ileride
     *  sunucu tarafı etiketleri buradan okunabilir. */
    setMeta(_wireMeta) {
      // Şu an bir iş yapmıyor: düğüm etiketleri ve açıklamaları AGENTS
      // dizisinde sabit. Sunucu taraflı etiket desteği eklendiğinde
      // burada güncellenir.
    },

    /** Yeni bir koşu başladığında çağrılır. Demo kipini kapatıp gerçek
     *  devir verisini çekmeye başlar. */
    setRunId(id) {
      runId = id;
      lastHandoffCount = 0;
      if (id) {
    ...
    /** Her SSE `state` çerçevesinde çağrılır. Devir defteri koşu
     *  ilerledikçe büyüyor, ekran da onunla büyümeli. */
    applyState() {
      refresh();
    },
