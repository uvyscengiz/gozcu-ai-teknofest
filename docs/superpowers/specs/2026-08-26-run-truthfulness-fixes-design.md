...
2. **Hiçbir siren çalmadı, hiçbir ekip yola çıkmadı.** 6 `dispatch_medical` ve
   ...
   `open_safety_incident` çağrısının hepsi uydurma `episode_id` ile reddedildi.
   Gerçek bir devrilme için sahaya tek bir mock müdahale bile ulaşmadı.
...
`Supervisor.escalate` okuyor (`NO_DESCRIPTION_NOTE`). Üç tüketici okumuyor:
...
- `embed_episode`: `summary_source == "fallback"` olan epizot gömülmez
  ...
  sözleşmesinin aynısı). Kaynaşma zinciri özeti iyileştirdiğinde
  (`update_episode` model özetiyle üstüne yazıyor) kapanıştaki gömme zaten
  temiz metni alır.
...
- `registry._incident_guard`: `NO_SUCH_EPISODE` reddi silinir — uydurma
  ...
  ret değil başarı biçimli bir cevap, ve jürinin saydığı defterde tek olaya
  6 İSG kaydı görünmesini engelliyor.
...
→ 2 (ilk yükseltme + koşu sonu süpürmesi); `nöbetçi.duyur` süresi gelişme
kipinde tek model çağrısına iner (7–42 s → ~5–15 s). `actions[]` kopyaları
kaynağında kurur (`build_output`'a ayrı tekilleştirme eklenmez — YAGNI).
...
- Taşma kuralı: **ilk 24 + son 24** (`merged[:HALF] + merged[-HALF:]`,
  sınırda çakışma ayıklanarak). Baş, olayın nasıl başladığını korur; son,
...
- Raportör çağrısı `max_tokens=16384` geçer: koşu başına bir kez çalışır,
  girdisi en büyük prompt (epizotlar + riskler + defter + diyalog) ve 4096'yı
  aştığı ölçüldü. En kötü hâl maliyeti tek çağrıya sınırlı.
...
| Madde | Kırmızı test |
|---|---|
| §1 | Yedek özetli epizotla `_digest` çıktısında arıza metni GEÇMEZ; `assess_risk` prompt'unda geçmez ve arama anlardan kurulur; `embed_episode` `False` döner |
| §2 | Bilinmeyen bölgeyle `dispatch_medical` → `state="dispatched"`; `site_alarm` → `siren_state="active"`; uydurma `episode_id` → kayıt açılır; aynı epizoda ikinci kayıt → `duplicate` + ilk `record_no`; `escalate` mesajında gerçek `episode_id` |
...
| §6 | `assess_risk` devri `end_ts` damgalı; kartın damgası yükseltme anı |
